...
class Verifier:
    ...
    def compare_metrics(self, probe):
        ...
        typer_user = input(
            "Are you enrolled? if yes type your username, else type Imposter(guest):  ")
        ...
        bayes_score = verifier.compute_logits(data_tensor, probe_tensor) / (len(verifier.input_word) - 1)
        norm_score = verifier.compute_norm(differences) / (len(verifier.input_word) - 1)

        # thresholds based on evaluation results
        bayes_threshold = -4.220 
        norm_threshold = 0.225 

        # Compare the scores with the thresholds
        if (bayes_score) < naive_bayes_threshold and norm_score < norm_threshold:
